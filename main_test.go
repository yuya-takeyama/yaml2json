...
func TestSingleYAMLFromStdin(t *testing.T) {
	...
	file, err := os.Open("fixtures/single.yaml")
	if err != nil {
...
func TestMultipleYAMLsFromStdin(t *testing.T) {
	cmd := exec.Command("go", "run", "main.go")
	stdout := &bytes.Buffer{}
	stderr := &bytes.Buffer{}
	cmd.Stdout = stdout
	...
	file, err := os.Open("fixtures/multiple.yaml")
	if err != nil {
		t.Errorf("failed: %v", err)
	}
